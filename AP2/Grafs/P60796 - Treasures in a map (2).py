...
def cal_dist(map: TreasureMap, ini: Point) -> int:

    rows, cols = len(map), len(map[0])

    dq: deque[tuple[Point, int]] = deque() # Point: point, int: dist
...
def main() -> None:
    ...
    dist = cal_dist(map, Point(c, r))
    print(f'minimum distance: {dist}') if dist else print("no treasure can be reached")
